...
otu.pooled<-as.data.frame(matrix(nrow=538,ncol=12))
...
otu.pooled.RESS<-as.data.frame(matrix(nrow=497,ncol=24))
...
print(result.locality.RESS.lower.Artificial)
...
# Unconstrained analysis - MDS (aka PCoA) ----
...
ggsave(plot=COMM_PCoA.plot, "Whole_community/Unconstrained_MDS_loc_habitat.LEGEND.pdf",
       dpi=300,width=6,height = 6)

ggsave(plot=COMM_PCoA.plot, "Whole_community/Unconstrained_MDS_loc_habitat.pdf",
       dpi=300,width=6,height = 6)
...
## Add indicator to the PCOA plot ---- this was eventually not included in the mauscript
...
RESS_PCoA.plot<-plot_ordination(RESS, RESS_PCoA, color= "Locality", 
                                shape="Type_habitat_broad")+
  geom_point(size=4, alpha=1, aes(color=Locality))+
  scale_color_manual(values = loc.cols)+ 
  labs(title = paste0(" PCoA - RESS"))+fig+
  theme(legend.position = "none")

RESS_PCoA.plot

ggsave(plot=RESS_PCoA.plot, "Local/Unconstrained_MDS_loc_habitat.pdf",
       dpi=300,width=6,height = 6)
...
COMM_metaVenn<-COMM_metaVenn %>% dplyr::select(sample_name,Type_habitat_broad)
...
COMM_tax %>% group_by(Trait1) %>% summarise(freq=(n()*100)/538) 
COMM_tax %>% group_by(Trait1_3rd_lev_fish) %>% summarise(freq=(n()*100)/538)
...
summ.comm$cont$importance # importanc eof axes
...
tax %>% group_by(Trait1) %>% summarise(freq=(n()*100)/497)
tax %>% group_by(Trait1_3rd_lev_fish) %>% summarise(freq=(n()*100)/497)
...
wilcox.test(cap.RESS.2$CAP1~cap.RESS.2$Trait2_Worms) 
...
length(taxNIS$FARTA[taxNIS$NINIS_Native%in%"NIS"]) #43
...
tax<-as.data.frame(tax_table(RESS))
length(tax$FARTA[tax$NINIS_Native%in%"NINIS"]) #120 
...
boxplot.proportions
ggsave(plot=boxplot.proportions, "NIS/NIS_proportion_across_localities_no_habitat_distinction.pdf",
...
ggsave(plot=NIS.rich.plot,"NIS/NIS_richness_across_localities.pdf",
...
metaNIS.Venn<-metaNIS %>% dplyr::select(sample_name,Type_habitat_broad)
...
ggsave(plot=NIS_PCoA.plot, "NIS/Unconstrained_MDS_loc_habitat.pdf",
      dpi=300,width=6,height = 6)
...
indicspecs.lines.nNIS<-capture.output(print(summary(indicspecs.NIS))) 
...
NIS.tax.trait2 %>% group_by(Trait2_Worms)%>% summarise(freq=(n()*100)/39)
