## Script used to process OTU tables to generate the Species Occurrence Table used for the diversity analyses.

# Data used: OTU tables generated for each marker (12S, 18S, COI) by the pipeline at https://gitlab.ifremer.fr/abyss-project/abyss-pipeline 
# and stored in phyloseq objects.

# In this script:
# 1) Rarefying each OTU table to the minimum number of reads
# 2) Selecting only OTU assigned to Animalia
# 3) Validate taxonomic classification using Worms and remove all non-marine organisms
...
library(broom)
library(worrms)
library(car) #for Levene test of homeosedasticity
library(tibble)
library(ggpubr)
library(MicEco)
library(readxl)
...
# Phyloseq objects with 12S, 18S, and COI
names(asv_phyloseq_objects) <- c("12S", "18S", "COI")
names(otu_phyloseq_objects) <- c("12S", "18S", "COI")

sum(sample_sums(asv_phyloseq_objects$COI))
...
summer.samples<-sample.names(otu_phyloseq_objects_summer$COI)

coi<-read_xlsx("~Copia de dataset_med_int_ext_readtrack.xlsx", sheet=1)
coi.sub<-subset(coi,sample%in%summer.samples)
sum.coi<-colSums(coi.sub[,-1])

df.12s<-read_xlsx("~Copia de dataset_med_int_ext_readtrack.xlsx", sheet="12S")
d.12s.sub<-subset(df.12s,sample%in%summer.samples)
sum.12s<-colSums(d.12s.sub[,-1])

df.18s<-read_xlsx("~Copia de dataset_med_int_ext_readtrack.xlsx", sheet="18S")
d.18s.sub<-subset(df.18s,sample%in%summer.samples)
sum.18s<-colSums(d.18s.sub[,-1])



# How many Eukaryotes ?

euk.12s<-subset_taxa(otu_phyloseq_objects_summer$`12S`, Kingdom%in%"Eukaryota")
euk.18s<-subset_taxa(otu_phyloseq_objects_summer$`18S`, Kingdom%in%"Eukaryota")
euk.coi<-subset_taxa(otu_phyloseq_objects_summer$COI, Kingdom%in%"Eukaryota")
...
#check for sequencing depth per sample
# Obtain total number of reads for each marker and for summer samples only

print("sum")
for (i in seq_along(otu_phyloseq_objects_summer)){
  mean<-mean(sample_sums(otu_phyloseq_objects_summer[[i]]))
  sum<-sum(sample_sums(otu_phyloseq_objects_summer[[i]]))
  print(names(otu_phyloseq_objects_summer[i]))
  print(sum)
}
...
# Plot the distribution of reads per sample for all phyloseq objects
plot_distributions <- function(phyloseq_list, title_prefix = "Reads Distribution: ") {
  par(mfrow = c(1, length(phyloseq_list)))  # Plot multiple objects side by side
  
  for (i in seq_along(phyloseq_list)) {
    ps <- phyloseq_list[[i]]
    reads_per_sample <- sample_sums(ps)  # Extract reads per sample
    hist(reads_per_sample, 
         main = paste0(title_prefix, names(phyloseq_list)[i]), 
...
# Apply to asv_phyloseq_objects
depth_by_sample<-plot_distributions(otu_phyloseq_objects_summer, title_prefix = "OTU Reads Distribution: ")

# Barplot with bars colored by habitat

plot_depth_by_type_habitat <- function(phyloseq_list, title_prefix = "Seauencing Depth: ") {
  par(mfrow = c(1,length(phyloseq_list)))  # Plot multiple objects one after the other 
  
  for (i in seq_along(phyloseq_list)) {
    phyloseq <- phyloseq_list[[i]]
    
    reads.per.sample<-sample_sums(phyloseq) %>% data.frame()
    
    reads.per.sample$sample_name<-rownames(reads.per.sample)
    
    meta<-sample_data(phyloseq)%>% data.frame()
    
    meta.reads<-left_join(meta,reads.per.sample, by = "sample_name")
    
    meta.reads<-meta.reads %>% arrange(desc(.))
    
    meta.reads<-meta.reads %>% mutate(col=case_when(Type_habitat_broad %in% "Artificial"~"red", Type_habitat_broad %in% "Natural"~"green"))
    
    barplot(meta.reads$., ylab = "Sequencing depth", 
            xlab="Samples",
            main = paste0(title_prefix, names(phyloseq_list)[i]),
            col=meta.reads$col) +abline(h=min(meta.reads$.),col="blue")
  }
}

# Apply to asv_phyloseq_objects
dpth_by_habitat<-plot_depth_by_type_habitat(otu_phyloseq_objects_summer, title_prefix = "OTU Sequencing depth by Habitat type: ")

# Rarefaction curves

rarefaction_curves <- function(phyloseq_list, title_prefix = "Seauencing Depth: ") {
  ...
  for (i in seq_along(phyloseq_list)) {
    ...
    meta.reads<-meta.reads %>% 
      mutate(col=case_when(Type_habitat_broad %in% "Artificial"~"red", Type_habitat_broad %in% "Natural"~"green"))
    
    
    rarecurve(as.data.frame(t(otu_table(phyloseq))),step=1000,
              label= FALSE,
              ylab="OTUs",
              main = paste0(title_prefix, 
                            names(phyloseq_list)[i]),
              col=meta.reads$col,
              xlab="Sequencing depth")
    
    abline(v=min(meta.reads$.),col="blue") 
  }
}
...
# Apply to asv_phyloseq_objects
set.seed(1234)
rare.plot<-rarefaction_curves(otu_phyloseq_objects_summer, title_prefix = "OTU rarefaction curves by Habitat type: ")
...
print("12S")
length(taxa_names(subset_taxa(rarefied_otu_summer_objects$`12S`, Kingdom%in%"Eukaryota")))
print("18S")
length(taxa_names(subset_taxa(rarefied_otu_summer_objects$`18S`, Kingdom%in%"Eukaryota")))
print("COI")
length(taxa_names(subset_taxa(rarefied_otu_summer_objects$COI, Kingdom%in%"Eukaryota")))

# How many unassagned taxa ?
print("12S")
length(taxa_names(subset_taxa(rarefied_otu_summer_objects$`12S`, Kingdom%in%"no data")))
print("18S")
length(taxa_names(subset_taxa(rarefied_otu_summer_objects$`18S`, Kingdom%in%"no data")))
print("COI")
length(taxa_names(subset_taxa(rarefied_otu_summer_objects$COI, Kingdom%in%"no data")))
...
# in the animalia.list.worms the coñumn "scientificname" correspond to the accepted phylum
...
# Prune OTUs from asv_phyloseq_filtered_objects
# 
# Old script pre Cloe check 
# rare_otu_phyloseq_Marine.animalia <- lapply(rare_otu_phyloseq_objects.animalia, function(ps) {
#   remove_otus_from_list(ps, otus_to_remove)
# })
# 
# # Print final number of OTUs assogned to marine organisms
# dim(tax_table(rare_otu_phyloseq_Marine.animalia$`12S`))[1] # 537
# dim(tax_table(rare_otu_phyloseq_Marine.animalia$`18S`))[1] # 368
# dim(tax_table(rare_otu_phyloseq_Marine.animalia$`COI`))[1] # 409
...
# We merge OTU with the same FARTA ----
...
# 
# save(combined_taxa_phyloseq_WormsAcc,file="Preparation_combined_dataset/combined_taxa_phyloseq_taxonomy_WORMS_accepted.RData")

# Subset only the samples and species from summer campaign
...
# A few manual changes to the dataset, following revision 

# There is an error with the genus Zachsia : this is labelled as "Species" but instead is "Genus" - so it must be removed from the ps object 
combined_taxa_ps_summer.sp<-prune_taxa(as.data.frame(tax_table(combined_taxa_ps_summer.sp))[,"Species"]!="Zachsia",combined_taxa_ps_summer.sp)

# By suggestion of coauthor Marc Bouchoucha we recontrolled the classification of Watersipora subtorquata done by BLAST using MIDORI 2.
# In la seyne sur mer both W. subatra and subtorquata have been identified morphologically. Therefore the check. 


# From our check on the OTU tables of each markers:
#  Of COI OTUs:
# OTU_127 best hit in BLAST to Watersipora subatra. This OTU was found in two out of the three port sites in Seyne-sur-mer: Mar0621_SEY_P4 (4 reads) and in Mar0621_SEY_P6 (28 reads). Note that this OTU was not found in any other locality across the sample pool.
# OTU_1788: Watersipora subtorquata also found in port site in Seyne-sur-mer (Mar0621_SEY_P6) 
# OTU_2386: Watersipora subtorquata found in other localities than Seyne-sur mer or Toulon

# Gauf et al 2023  https://doi.org/10.3391/ai.2023.18.3.108128 was consuted to double checj tha tthe three COI OTU matched with accession numbers assigned to W. subatra or subtorquata. 
# The affiliation of the accesion numbers matching our OTUs to different clades in Gauf et al tree was used to finally asisgn the COI OTU to the correct species. 
# From this check we noted that also OTU_1788 must be assigned to W. subatra, since the sequence matching this OTU belogn to CLADE 5A in Gauf three which is classified as subatra.

# Of the 18S OTU:
#   
#   OTU_484: Watersipora subtorquata only OTU found in Toulon (Mar0721_TOU_2) and also in two port sites in Seyne sur Mer (Mar0621_SEY_P4,  Mar0621_SEY_P5).


tax<-as.data.frame(tax_table(combined_taxa_ps_summer.sp))
otu<-as.data.frame(otu_table(combined_taxa_ps_summer.sp))

sp.occ.tbl<-read.csv2("~Species_occurrence_table_Watersipora_check.csv")
sp.occ.tbl.tax<-sp.occ.tbl %>% select(!contains("Mar"))
sp.occ.tbl.otu<-sp.occ.tbl %>% select(contains("Mar0"), OTU_seed)

watersipora<-subset(sp.occ.tbl.tax, Genus_Worms_Accepted%in%"Watersipora")
tax.new<-tax%>% subset(!Genus%in%"Watersipora")

watersipora.names<-colnames(watersipora)
tax.new.names<-colnames(tax.new)

sd<-setdiff(tax.new.names,watersipora.names)

watersipora[sd]<-NA


tax.new<-rbind(tax.new,watersipora)

# # write.csv2(tax.new,file="taxonomy_newWaterispora.csv")
# tax.new<-read_xlsx("taxonomy_newWaterispora.xlsx")
rownames(tax.new)<-tax.new$OTU_seed

# OTU table
waterispora.otu<-subset(sp.occ.tbl.otu, OTU_seed%in%c("18S_OTU_484","COI_OTU_127"))
rownames(waterispora.otu)<-NULL

otu$otu<-rownames(otu)
otu.new<-subset(otu,!otu%in%"18S_OTU_484")%>% select(!otu)

waterispora.otu<- otuwaterispora.otu<- waterispora.otu%>%column_to_rownames(var="OTU_seed")
otu.new<-rbind(otu.new,waterispora.otu)


#import in phyloseq object
tax.new<-as.matrix(tax.new)
names<-rownames(otu.new)
otu.new<-as.matrix(otu.new)

combined_taxa_ps_summer.sp<-phyloseq(otu_table(otu.new,taxa_are_rows = TRUE),
